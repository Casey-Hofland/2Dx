Shader "Skybox/Procedural - 360" {
    ...
    Pass {
        ...
        // Quaternion inverse
        // http://mathworld.wolfram.com/Quaternion.html
        float4 qinv(float4 quat)
        {
            float4 conjugate = float4(-quat.xyz, quat.w);
            float magnitude = sqrt(mul(quat, quat));
            return float4(conjugate / (magnitude * magnitude));
        }
        ...
        v2f vert(appdata_t v)
        {
            ...
            float3 rotated = RotateVector(_Rotation, v.vertex);
            ...
            float3 worldSpaceLightPos0Rotated = RotateVector(qinv(_Rotation), _WorldSpaceLightPos0);
            if (eyeRay.y >= 0.0)
            {
                // Sky
...
//              for(int i=0; i<int(kSamples); i++)
                {
                    ...
                    float lightAngle = dot(worldSpaceLightPos0Rotated, samplePoint) / height;
            ...
            else
            {
                ...
                float lightAngle = dot(worldSpaceLightPos0Rotated, pos);
            ...
            OUT.skyColor = _Exposure * (cIn * getRayleighPhase(worldSpaceLightPos0Rotated, -eyeRay));
        ...
        half4 frag(v2f IN) : SV_Target
        {
            ...
            // if y < 0 [eyeRay.y > 0] - sky
                ...
                {
                    float3 worldSpaceLightPos0Rotated = RotateVector(qinv(_Rotation), -_WorldSpaceLightPos0);
                    col += IN.sunColor * calcSunAttenuation(worldSpaceLightPos0Rotated, -ray);
                }
            #endif
